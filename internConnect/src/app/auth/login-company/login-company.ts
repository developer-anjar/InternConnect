...
userType: string = 'student';
  ...
  sendForgotEmail() {
    ...
    this.http.post('http://localhost:8080/api/company/forgot-password', { email: this.forgotEmail }).subscribe({
  ...
  onLogin() {
    if (this.loginForm.valid) {
      ...
      this.http.post('http://localhost:8080/api/company/login', loginData).subscribe({
        next: (res: any) => {
